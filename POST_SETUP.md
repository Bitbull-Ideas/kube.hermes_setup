...
- Internal/Confidential storage: private repositories in Bitbull-Ideas, or another private location explicitly approved by the requesting user
...
Requirements:
1. Load the Hermes Agent, team-policy, and Git workspace guidance that is available before acting.
...
4. Do not install the public team-policy-template unchanged. Adapt it into an operational skill named team-policy.
5. Preserve source attribution, the exact source commit, and GPL-3.0 licensing in the adapted skill.
...
7. Install the adapted skill for the active Hermes profile. Determine the profile's skill path from the live Hermes configuration rather than assuming another user's home directory.
8. Use restrictive permissions for the installed skill directory and files.
9. Verify that team-policy is discoverable and loadable without exposing credentials.
10. Do not publish, merge, rotate credentials, change access controls, or perform destructive cleanup unless separately requested.
11. After validation, offer the exact cleanup/archive action for the clean source clone and wait for approval before moving it to /workspace/git_archive/.
12. Report installed files, source commit, license, checks run, new-session or reload requirement, and limitations.
