...
Fix in this installer: `prepare-browser-cli` builds a complete Node/npm/npx candidate under `/opt/data/node/runtimes/<hash>`, records installer-owned integrity metadata, validates Node and both npm entry points off-path, then atomically switches `/opt/data/node/current`. The public `/opt/data/node/bin/{node,npm,npx}` launchers resolve that pointer, prepend only the selected runtime's private library directory, and preserve the WebUI image's inherited `LD_LIBRARY_PATH`. A failed dependency check or malformed candidate leaves every active command unchanged; a corrupt retained same-key runtime is rebuilt to a new validated generation from the trusted Agent image. The WebUI container itself does not receive a replacement loader-path environment value. `libatomic.so.1` is copied only when the Agent Node ELF resolves it; an unresolved required library fails initialization before activation.
